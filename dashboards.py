...
fig_data = px.bar(
    df_filtrado, x="Data", y="Total", color="Cidade", title="Faturamento por Dia"
)
col1.plotly_chart(
    fig_data, use_container_width=True
...
col4.plotly_chart(
    fig_tipo_pagamento, use_container_width=True
)  # Exibe o gráfico na quarta coluna

# Gráfico 5: Avaliação média das filiais (barra)
total_cidade = (
    df_filtrado.groupby("Cidade")[["Avaliacao"]].mean().reset_index()
)  # Agrupa por cidade e calcula a média da avaliação
fig_avaliacao = px.bar(
    total_cidade, y="Avaliacao", x="Cidade", title="Avaliação das Filiais"
)
col5.plotly_chart(
    fig_avaliacao, use_container_width=True
